        ...
        const loadDashboardData = async () => {
            try {
                ...
                const fetchProfileOnce = async () => {
                    try {
                        const data = await fetchWithRateLimit("https://music-ml-server.onrender.com/auth/profile");
                        if (data && isMounted) {
        ...
        return (
            <div className="flex flex-col min-h-screen bg-[var(--theme-bg)] items-center justify-center text-center p-8">
                <div className="bg-[var(--theme-panel)] border border-[var(--theme-border)] rounded-[40px] p-12 flex flex-col items-center">
                    ...
                    <a href="https://music-ml-server.onrender.com/auth/login" className="px-6 py-3 rounded-full bg-[#D1F26D] text-black font-bold uppercase text-xs tracking-widest hover:scale-105 transition-transform">
                        Connect Spotify
                    </a>
                </div>
            </div>
